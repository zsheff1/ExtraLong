# The following code creates the file freesurferLongitudinal_aseg.table.txt, which combines the aseg.stats files of every subject into a single table for statistical analysis. To complete this task, a qdec-long table is created, with a structure similar to the example in the documentation of freesurfer's longitudinal pipeline.  
...
asegstats2table --qdec-long /$SUBJECTS_DIR/qdec/qdec_table.dat --skip --stats aseg.stats --tablefile $project/data/freesurferLongitudinal/qdec/aseg_freesurferLongitudinal.table.txt
